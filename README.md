...
- **訊息總結**：能夠將頻道中的訊息整理成一個清單，方便用戶查看和管理。
- **多次總結**：一定程度上解決了AI的「忘記之前的對話內容」的問題，能夠在多次總結中保留之前的訊息內容。
- **多頻道監控**：能夠同時監控多個頻道，並分別總結每個頻道的訊息。
- **效能優化**：為了節約效能，機器人不會紀錄每個頻道，只有在使用者要求監控後才會開始紀錄該頻道的訊息。
